...
def _drafting_now() -> set[str]:
    return {j.get("book_catalog") for j in list_jobs(active_only=True)
            if j.get("kind") in ("full_draft", "childrens_book") and j.get("book_catalog")}
...
def advance(max_starts: int = 2) -> dict:
    ...
    for sid, members in by_series.items():
        ...
        prev = None
        for m in members:
            ms = (m["data"].get("manuscript") or {}).get("status") or "idea"
            if ms in STARTED or m.get("status") == "generating":
                prev = m; continue
            # first unstarted book: its predecessor must be drafted
            if prev is None or ((prev["data"].get("manuscript") or {}).get("status") or "") not in ("drafted", "editing", "locked", "complete"):
